...
app.get('/api/client/:clientId/bonus-transactions', async (req, res) => {
    try {
        ...
        // Filter FreeSpin, Pay Client Bonus, and Corrections after deposit
        const bonusTransactions = transactions.filter(tx => {
            ...
            if (tx.DocumentTypeId === 301) {
                return true;
            }
            // Correction Down (DocumentTypeId 302)
            if (tx.DocumentTypeId === 302) {
                return true;
            }
            return false;
        }).map(tx => {
...
// AUTO-APPROVAL ENDPOINTS DEPRECATED
// All rule logic is now handled by Unified Rule Engine (see /api/rules/*)
